...
async def test_create_user_with_default_habit_success(
    async_user_manager, username, email, nickname, password
):
    """Integration test: successfully create user with default habit."""
    user = await async_user_manager.create_user_with_default_habit(
        username, email, nickname, password
    )
    ...
    habits = await async_user_manager.service.habit_repo.get_all_habits_for_user(
        user.user_id
    )
    assert len(habits) == 1
...
async def test_create_user_with_default_habit_rollback(
    async_user_manager, fake_user_data
):
    ...
    with patch(
        ...
        with pytest.raises(ValueError, match="Simulated failure"):
            await async_user_manager.create_user_with_default_habit(
                username, email, nickname, password
            )
...
@pytest.mark.asyncio
async def test_create_user(async_user_manager, fake_user_data):
...
@pytest.mark.asyncio
async def test_delete_user(async_user_manager, fake_user_data):
